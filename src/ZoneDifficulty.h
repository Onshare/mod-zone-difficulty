...
uint32 const ITEMTYPE_MISC = 1; //饰品
uint32 const ITEMTYPE_CLOTH = 2; // 布甲
uint32 const ITEMTYPE_LEATHER = 3;  //皮甲
uint32 const ITEMTYPE_MAIL = 4; //锁甲
uint32 const ITEMTYPE_PLATE = 5; //板甲
uint32 const ITEMTYPE_WEAPONS = 6; //武器
...
public:
    ...
    void GiveMythicmodeItem(Map* map, uint32 type);
    ...
    std::map<uint32, uint32> HeroicWLKQuestMapList;
    ...
    typedef std::map<uint32, std::vector<ZoneDifficultyRewardData> > ZoneDifficultyAllRewards;
    ZoneDifficultyAllRewards AllRewards;
};
